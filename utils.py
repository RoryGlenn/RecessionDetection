...
def get_data_after_date(
    ...
    """
    The function `get_data_after_date` takes a DataFrame, removes any rows with missing values, and
    returns data from a specified date up to one year ago.

    :param data: The `data` parameter is expected to be a pandas DataFrame containing time series data
    with a datetime index
    :type data: pd.DataFrame
    :param years: The `years` parameter in the `get_data_after_date` function specifies the number of
    years to go back from the most recent date in the data. This parameter is set to 1 by default,
    meaning that the function will return data starting from one year ago up to the most recent date in,
    defaults to 1
    :type years: int (optional)
    :param days: The `days` parameter in the `get_data_after_date` function specifies the number of days
    to subtract from the most recent date to determine the starting point for the data retrieval. By
    default, it is set to 0, meaning that if no specific number of days is provided, the function will,
    defaults to 0
    :type days: int (optional)
    :return: The function `get_data_after_date` takes a DataFrame `data` as input, along with optional
    parameters `years` and `days` with default values of 1 and 0 respectively.
    """
...
def get_recession_start_end_list(recdf):
    """
    The function `get_recession_start_end_list` takes a DataFrame containing recession data and returns
    a list of tuples representing the start and end dates of each recession period.

    :param recdf: The function `get_recession_start_end_list` takes a DataFrame `recdf` as input and
    returns a list of tuples where each tuple represents the start and end dates of a recession period
    :return: The function `get_recession_start_end_list` returns a list of tuples, where each tuple
    contains the start and end dates of a recession period detected in the input recession dataframe
    `recdf`.
    """
    reclist = []
...
def plot_data(symbol, title, years, plot_recession_dates=True, hline=None, units=None):
    """
    The function `plot_data` retrieves and plots time series data, with options to include recession
    dates and horizontal lines.

    :param symbol: The `symbol` parameter is typically a string that represents the symbol or code for
    the financial data series you want to plot. It is used to retrieve the data for the specified symbol
    from a data source like FRED (Federal Reserve Economic Data)
    :param title: The `title` parameter is a string that represents the title of the plot that will be
    generated by the `plot_data` function. It is used to provide a descriptive title for the plot to
    help users understand the data being visualized
    :param years: The `years` parameter in the `plot_data` function is used to specify the number of
    years of data you want to plot. This parameter determines how much historical data will be included
    in the plot for the given symbol
    :param plot_recession_dates: The `plot_recession_dates` parameter in the `plot_data` function is a
    boolean flag that determines whether recession dates should be plotted on the graph. If set to
    `True`, the function will include recession dates on the plot. If set to `False`, recession dates
    will not be included on, defaults to True (optional)
    :param hline: The `hline` parameter in the `plot_data` function is used to specify a horizontal line
    to be added to the plot. This can be helpful for visualizing a specific value or threshold on the
    plot. You can pass a numerical value to `hline` to indicate where the horizontal line
    :param units: The `units` parameter in the `plot_data` function is used to specify the units in
    which the data should be retrieved from the FRED API. If the `units` parameter is provided, it will
    be passed to the `fred.get_series` function to fetch the data in the specified units
    """
    data = fred.get_series(symbol, units=units) if units else fred.get_series(symbol)
...
def plot_it(data_df, title, plot_recession_dates=False, hline=None):
    """
    The function `plot_it` generates a line plot from a DataFrame with the option to include recession
    dates and a horizontal line.

    :param data_df: The `data_df` parameter is a DataFrame containing the data to be plotted. It should
    have a column named "date" representing the dates for the x-axis, and other columns containing the
    data to be plotted on the y-axis
    :param title: The `title` parameter is a string that represents the title of the plot that will be
    displayed. It is used to provide a descriptive title for the plot to help users understand the data
    being visualized
    :param plot_recession_dates: The `plot_recession_dates` parameter is a boolean flag that determines
    whether to plot recession dates on the graph. If set to `True`, the function will fetch recession
    data and overlay shaded rectangles on the plot to highlight the periods of recession. If set to
    `False`, recession dates will not be, defaults to False (optional)
    :param hline: The `hline` parameter in the `plot_it` function is used to add a horizontal line to
    the plot at a specified y-value. This can be helpful for visualizing a specific threshold or
    reference value on the plot
    """
...
def get_regression_data(
    ...
    X_train, y_train, X_test, _ = train_test_split_02(
